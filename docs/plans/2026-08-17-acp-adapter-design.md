...
The package exposes DeepSeek Harness agents as a stable ACP v1 server over JSON-RPC stdio. It must preserve Harness ownership and lifecycle semantics while presenting the richer editor experience established by `claude-agent-acp` and `codex-acp`: selectable models, selectable reasoning effort, assistant and thought content grouped by message, visible tool calls and results, plans, permission requests, cancellation, and session metadata. The first release creates fresh in-memory sessions only. Persisted session listing, load, resume, fork, and deletion remain a later capability because they require a separate persistence contract and transcript replay policy.

The implementation targets Node.js 22.19 or newer within the Node.js 22 release line, or Node.js 24 and newer, TypeScript in strict ESM mode, ACP protocol version 1, and `@agentclientprotocol/sdk` 1.3.x. It uses Harness public services and events instead of importing the concrete agent loop. Model catalogs are advisory, so the current configured route remains usable and visible even if it is absent from the provider catalog. Stdout is reserved exclusively for ACP frames; diagnostics use stderr or the Harness logger.
...
The update projector is pure where possible. Committed `assistant/message` text and reasoning blocks become ACP message and thought chunks that share the Harness message id. Raw `assistant/chunk` events stay internal because a failed provider attempt can emit them before Harness retries, while ACP v1 has no operation that retracts already delivered content. `tool/call` and `tool/result` become correlated tool cards, with invalid JSON arguments retained as raw text rather than crashing presentation. `todo/write` becomes an ACP plan snapshot. Committed usage and title facts become session metadata updates when the ACP schema supports them. Permission requests are offered as one-shot allow and reject options, matching the Harness approval waterfall.
...
Unit tests cover model grouping, current out-of-catalog routes, reasoning-option replacement, prompt conversion, stop-reason mapping, tool projection, and malformed tool arguments. Protocol tests use the ACP SDK's in-memory streams to exercise initialize, new session, config changes, prompt updates, notification and request-level cancellation, permissions, and teardown through actual JSON-RPC handlers. Harness integration tests mount the real Agent registry and loop testkit with a scripted LLM, proving that the adapter creates agents through `ctx.agents`, switches models through `installModelSelection`, and projects only committed assistant content after a provider retry.
